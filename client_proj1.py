...
def set_status_code(rfc_no,request_version,client_version,client_dir,peer_name,clientname):
	if str(peer_name) == str(clientname):
		return 400
        ...
        while 1:
			...
			if re.search('GET RFC .*',rfc_request):
					...
					peer_name = str(request_header[0][2])
        	                	status_code = set_status_code(rfc_no,request_version,client_ver,client_dir,peer_name,clientname)
        	        	else:
					rfc_no = 0
        	                	status_code = 400
...
def get_file_from_peer(peer_addr,peer_port,rfc_no,clientname,client_ver,client_os,client_dir):
	...
	#print "Peer connected"
        ...
        receive_rfc(peerSocket,client_dir,rfc_no)
	peerSocket.close() 	
		...
		if msg:
			...
			if rfc_reply_status:
				...
				if status_code == 200:
					...
					get_file_from_peer(peer_addr,peer_port,rfc_no,clientname,client_ver,client_os,client_dir)
	elif choice == 3:
		'''list all the rfc'''
